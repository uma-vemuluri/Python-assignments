...
#MEDIUM QUESTIONS
#If you append a list [10, 20] to an existing list [1, 2],  what is the total number of elements in thefinal list?
l = [1, 2]
l.append([10, 20])
print(l)  # [1, 2, [10, 20]]
print(len(l))  #3
...
insert(0, x) is slower than append(x) for large lists because insert(0, x) requires shifting all existing elements in the list one position to the
...
For large lists, shifting every element makes it slower.
